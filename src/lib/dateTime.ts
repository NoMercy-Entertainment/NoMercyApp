...
export const humanTimeExtended = function (t: Function, dateTime: Date | number, short = false) {
...
export const randomDate = () => {
    ...
    const randomEpoch = Math.floor(Math.random() * (now - oneWeekAgo) + oneWeekAgo);
...
export const timeAgoShort = (epoch: Date | number) => {
    ...
    const now = Date.now();
    const secondsAgo = Math.floor((now - (epoch > 1000000000000 ? epoch : epoch * 1000)) / 1000); // Check if epoch is in milliseconds
...
export const userTime = (time: Date | number) => {
