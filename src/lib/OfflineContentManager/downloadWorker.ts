import type { Attributes, Manifest } from 'm3u8-parser';
...
async function downloadContent(
	...
	try {
		...
		if (task.mediaPath.includes('.m3u8')) {
			const manifestResponse = await fetchWithAuth(
				task.mediaPath,
				baseUrl,
				token,
			);
			...
			const basePath = task.mediaPath.substring(
				0,
				task.mediaPath.lastIndexOf('/'),
			);
			...
			if (totalVideoSegments > 0) {
				...
				updateAssetProgress(
					'video',
					totalVideoSegments,
					completedVideoSegments,
				);

				// Only process segments that aren't already completed
				for (const manifest of videoManifests) {
					const videoBasePath = manifest.path.substring(
						0,
						manifest.path.lastIndexOf('/'),
					);
			...
			if (parsedManifest.mediaGroups?.AUDIO) {
				...
				for (const group of Object.values(parsedManifest.mediaGroups.AUDIO)) {
					for (const audio of Object.values(
						group,
					) as unknown as CustomManifestPlaylist[]) {
						if (!audio.uri)
							continue;
				...
				if (totalAudioSegments > 0) {
					...
					for (const manifest of audioManifests) {
						const audioBasePath = manifest.path.substring(
							0,
							manifest.path.lastIndexOf('/'),
						);
...
async function processQueue(
	...
	while (index < tasks.length) {
		...
		tasks.slice(index + 1).forEach((nextTask) => {
			if (
				!completedItems.get(nextTask.mediaId)
				&& !pausedItems.has(nextTask.mediaId)
			) {
		...
		try {
			...
			if (result === false) {
				...
				updateItemStatus(task.mediaId, 'paused');
				if (isPaused)
					return; // Exit queue if globally paused
...
function calculateProgress() {
	...
	const completedCount = Array.from(completedItems.values()).filter(
	...
	originalTasks.forEach((task) => {
		if (completedItems.get(task.mediaId)) {
			...
			if (task.subtitlePaths?.length)
				requiredAssets.add('subtitle');
			if (task.fontPaths?.length)
				requiredAssets.add('font');
			if (task.previewPaths?.length)
				requiredAssets.add('preview');

			// Calculate progress based on completed required assets
			const assets = completedAssets.get(task.mediaId)!;
			const completedRequired = Array.from(requiredAssets).filter(type =>
				assets.has(type),
			).length;
		...
		switch (type) {
			...
			case 'resume':
				...
				const remainingTasks = originalTasks.filter(
					t => !completedItems.get(t.mediaId) && pausedIds.includes(t.mediaId),
				);
