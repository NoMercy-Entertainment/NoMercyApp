...
async function connectToHub() {
	if (dashboardSocket.value?.connection?.state === HubConnectionState.Connected)
		return;

	return dashboardSocket.value?.connection?.start().then(connected);
}

export async function stopDashboardSocket() {
	try {
		if (
		...
		return dashboardSocket.value?.connection
			?.stop()
			.then(disconnected)
			.catch(disconnected);
	}
...
export async function startDashboardSocket() {
	const accessToken = user.value?.accessToken;
