...
export class DesktopUIPlugin extends BaseUIPlugin {
	...
	createSubtitleSettingMenuButton(
		...
		subtitleSettingButtonText.textContent = this.player.localize(data.label);
		...
		if (data.property) {
			...
			this.player.on('set-subtitle-style', (style: SubtitleSettingAction) => {
				if (data.property !== style.property)
					return;
	...
	createSubtitleSettingActionButton(
		...
		subtitleSettingActionButtonText.textContent = this.player
			.localize(data.label)
			.toTitleCase();
	...
	createQualityMenuButton(
		...
		qualityButtonText.textContent = `${this.player.localize(
	...
	createProgressBar(parent: HTMLDivElement) {
		...
		this.bottomBar.addEventListener(
			...
			(e: any) => {
				...
				if (this.hasNoMercyConnect()) {
					this.sendNoMercyConnectCommand('seek', Number.parseInt(scrubTime.scrubTimePlayer.toString(), 10));
				}
				else {
	...
	createSeasonMenuButton(
		...
		buttonSpan.textContent = item?.seasonName
			? item?.seasonName
			: item?.season
	...
	createEpisodeMenuButton(
		...
		button.addEventListener('click', () => {
			...
			if (item.episode && item.season) {
				if (this.hasNoMercyConnect()) {
					this.sendNoMercyConnectCommand('episode', JSON.stringify({
						season: item.season,
	...
	createToolTip(parent: HTMLDivElement) {
		...
		this.tooltip.textContent = 'Play (space)';
	...
	private makeSubtitleMenuText(
		...
		if (data.property === 'fontFamily') {
			...
			subtitleSettingButtonValueText.textContent = fontFamilies.find(
				f => f.value === value,
			)!.name;
		}
		else if (data.property === 'edgeStyle') {
			subtitleSettingButtonValueText.textContent = edgeStyles.find(
				f => f.value === value,
			)!.name;
		}
		else {
			subtitleSettingButtonValueText.textContent = (
