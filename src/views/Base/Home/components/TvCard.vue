...
<template>
  <RouterLink :to="item.props.data.link.includes('watch') ? item.props.data.link : `${item.props.data.link}/watch`"
    v-if="item.props.data" :data-card="item.props.data" :index="index" @focus="scrollToCenter"
    ...
    <img :src="`${currentServer?.serverBaseUrl}/images/original${item.props.data.poster}?width=300`" alt=""
      loading="lazy" class="w-full h-auto aspect-poster object-cover object-center absolute inset-0">
    ...
    <div
      ...
      <p class="self-stretch flex-grow-0 flex-shrink-0 w-full text-[8px] text-left text-[#eff7ff]/[0.62]">
