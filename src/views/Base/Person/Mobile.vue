...
<template>
	...
	>
		...
		>
			<div class="flex justify-start items-end -mx-4 w-available h-[400px] relative gap-2">
				<div
					class="absolute flex flex-col justify-start items-end flex-grow w-available -mx-20 h-[400px] bg-cover bg-top"
					:style="backgroundUrl ? `background: linear-gradient(0deg, rgba(0,0,0,0.5) 0%, rgba(0,0,0,0.5) 100%), url('${backgroundUrl}') lightgray 50% / cover no-repeat` : undefined"
				/>
				<div class="absolute flex flex-col justify-start items-end flex-grow w-available -mx-20 h-[400px] bg-cover dark:bg-black/50" />
