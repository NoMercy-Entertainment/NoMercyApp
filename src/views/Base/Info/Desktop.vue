					...
					>
						...
						<div class="flex flex-col relative mx-auto w-full gap-4 rounded-lg p-4 max-w-screen-4xl justify-end min-h-[calc(100vh-6rem)]">
							...
							<div class="z-0 flex w-full flex-grow flex-col items-end justify-end gap-2">
								...
								>
									<TMDBImage v-if="data?.logo" :key="data?.logo" :auto-shadow="true" :path="data?.logo" :size="500"
										...
										class-name="relative bottom-0 h-auto w-auto p-4" type="logo"
									/>
								</div>
								...
								>
									...
									>
										<RouterLink :to="`/${data?.media_type}/${data?.id}/watch`" :aria-label="$t('Play')"
											:class="`relative h-auto m-auto mx-auto w-[65%] scale-95 cursor-default group/card block z-0 transitioning rounded-2xl aspect-poster overflow-clip select-none cover !shadow-none max-h-available  ${hasItem ? 'hover:!scale-100 hover:-translate-y-1 3xl:-mt-40' : ''}`"
											data-nav="true" data-nav-r="play" data-nav-reset="true"
										>
											<TMDBImage :key="data?.poster || 'poster'" :auto-shadow="true"
												:path="data?.poster" :color-palette="data?.color_palette?.poster" :size="760"
								...
								"
											...
											<div v-if="data" class="relative flex flex-shrink-0 flex-grow-0 items-center justify-start gap-4">
												...
												>
													<OptimizedIcon icon="play" class-name="w-6 text-red-dark-8" />
												...
												>
													<OptimizedIcon icon="film" class-name="w-6"
										...
										>
											...
											<InfoItem v-if="data?.genres" :data="data" title="Genres" key-name="genres" prefix="genres" />
											<InfoItem v-if="data?.writer" :data="{ writer: [data?.writer] }" title="Writer" key-name="writer"
												prefix="person"
											/>
											<InfoItem v-if="data?.creator" :data="{ creator: [data?.creator] }" title="Creator"
												key-name="creator" prefix="person"
											/>
											<InfoItem v-if="data?.director" :data="{ director: [data?.director] }" title="Director"
												key-name="director" prefix="person"
											/>
											<InfoItem v-if="data?.creators" :data="data" title="Creators" key-name="creators"
												prefix="person"
											/>
											<InfoItem v-if="data?.directors" :data="data" title="Directors" key-name="directors"
												prefix="person"
											/>
											<InfoItem v-if="data?.writers" :data="data" title="Writers" key-name="writers" prefix="person" />
											<InfoItem v-if="data?.keywords" :data="data" title="Keywords" key-name="keywords" />
											...
											>
												...
												>
													...
													<div v-if="data?.external_ids?.imdb_id" class="gap-1 children:whitespace-nowrap text-sm">
													...
													<div v-if="data?.external_ids?.tvdb_id" class="gap-1 children:whitespace-nowrap text-sm">
						...
						<MediaCarousel v-if="data?.recommendations && data?.recommendations?.length > 0"
							:color-palette="data?.color_palette" :data="data?.recommendations" title="Recommendations" type="poster"
						/>

						<MediaCarousel v-if="data?.similar && data?.similar?.length > 0" :color-palette="data?.color_palette"
							:data="data?.similar" title="Similar" type="poster"
						/>
					</div>

					<Trailer v-if="data?.videos && data?.videos?.length > 0 && trailerOpen" :key="trailerIndex"
						:increment-trailer-index="incrementTrailerIndex" :index="trailerIndex" :open="trailerOpen"
