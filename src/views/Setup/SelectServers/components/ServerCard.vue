...
<template>
	...
	>
		...
		>
			...
			>
				<DeviceIcon
					v-if="server.platform"
					:icon="server.platform"
					class-name="w-24 sm:w-32"
				/>
			</div>
		</div>
		<div
		...
		>
			...
			>
				<p
					class="flex-shrink-0 flex-grow-0 self-stretch text-sm font-semibold text-auto-12 line-clamp-1"
				>
					{{ server.name }}
				</p>
				<p
					class="flex-shrink-0 flex-grow-0 self-stretch text-xs text-auto-alpha-10"
				>
					{{ server.internal_ip }}
				</p>
			</div>
