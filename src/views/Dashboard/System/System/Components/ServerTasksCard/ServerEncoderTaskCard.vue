...
<template>
	...
	>
		<div class="relative flex w-full items-center justify-center gap-0.5">
			<p
				class="w-full flex-grow text-sm font-semibold text-auto-12 line-clamp-1 h-6"
        ...
        "
			>
				...
				>
					<OptimizedIcon v-if="data?.has_gpu" icon="gpu" class-name="w-[15%]" />
					<OptimizedIcon v-else icon="cpu" class-name="w-[15%]" />

					<OptimizedIcon v-if="data?.is_hdr" icon="hdr" class-name="w-[15%]" />
					<OptimizedIcon v-else icon="hdrOff" class-name="w-[15%]" />
				</div>
				...
				>
					<span
						class="group-hover/progress:hidden text-right font-bold text-auto-12 monospace"
					>
						{{ Math.floor(data?.progress ?? 0) }}%
					</span>
				</div>
			...
			<div class="flex h-full flex-grow flex-col items-start justify-evenly">
				...
				>
					<OptimizedIcon
						icon="stickyNote"
						class-name="relative w-4 text-auto-12"
					/>
