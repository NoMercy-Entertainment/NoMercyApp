...
function pushFile(file: FileItem) {
	if (
		files.value?.some(f => f.path === file.path && f.id === file.match?.id)
	) {
		...
		files.value = [
			...files.value,
			{ id: file.match?.id ?? 0, path: file.path },
		];
	}
}

function popFile(file: FileItem) {
	files.value = files.value.filter(
		f => f.path !== file.path && f.id !== file.match?.id,
	);
}
...
function handleSelectAll() {
	if (checkboxState.value) {
		...
		files.value = fileList.value.map(f => ({
			id: f.match?.id ?? 0,
			path: f.path,
		}));
	}
...
<template>
	...
	>
		<section class="flex w-full flex-col overflow-clip h-[65vh]">
			<!-- Form Fields -->
			<form class="mt-4 flex h-auto w-full flex-col items-start gap-4 sm:flex-row">
				<fieldset class="flex w-full flex-auto sm:w-64 gap-2">
			...
			<!-- File List -->
			<section class="mt-8 flex h-0 w-full flex-1 flex-col gap-2 overflow-clip relative">
				<ScrollContainer :auto-hide="false" :static="false" class-name="ml-2">
					...
					<figure
						v-else-if="!loading"
						class="absolute inset-0 flex h-auto w-full items-center justify-center flex-col gap-2"
					>
						<NoResultImage />
						<figcaption>{{ $t("No results") }}</figcaption>
					</figure>
		...
		<Modal
			...
			title="Add folder"
		>
			<section class="mx-auto flex w-full flex-col h-[45vh]">
