...
import {BluRayPlaylist, DriveState, Episode, RipperResponse} from "@/types/api/dashboard/ripper";
...
const submit = (e: Event) => {
  ...
  const json = Array.from(data).reduce((obj, [k, v]) => ({...obj, [k]: v}), {});
  console.log(json);
...
const handleSelect = (playlist: BluRayPlaylist) => {
  if (!discContents.value) return;
  ...
  const playlists = [
      ...discContents.value.metadata.bluRay_playlists?.filter(p => p.playlist_id !== playlist.playlist_id && p.video_tracks.length == 1) ?? [],
      playlist,
      ...discContents.value.metadata.bluRay_playlists?.filter(p => p.playlist_id !== playlist.playlist_id && p.video_tracks.length != 1) ?? [],
  ...
  const index = Array.from(discContents.value.metadata.bluRay_playlists)
...
const propagateSettings = (playlist: BluRayPlaylist) => {
  if (!discContents.value) return;

  const newContents =
      discContents.value.metadata.bluRay_playlists
          .filter(p => p.playlist_id !== playlist.playlist_id && p.video_tracks.length == 1)
          .map(p => {
...
const handlePlay = (playlist: BluRayPlaylist) => {
  ...
  serverClient().post(url)
      .then(() => {
        serverBaseClient().get(playlistUrl)
            .then(() => {
...
const handleChecked = (playlist: BluRayPlaylist) => {
  if (!discContents.value) return;
...
<template>
  <ion-page>
    <ion-content :fullscreen="true">
      <DashboardLayout title="Ripper" :gridStyle="4">
        <template v-slot:cta>

          <Select v-model="discContents" :options="discs"
                  :optionLabel="(episode: DriveState) => `${episode.path}: ${episode.label}`"
                  variant="filled" size="small" class="w-available min-w-96 mx-4"/>

          <template v-for="(disc, discIndex) in discs" :key="discIndex">
            <Button v-if="disc.open" id="open" @click="handleClose(disc.path)" variant="contained" color="theme"
                    type="submit">
              {{ $t('Close') }} {{ disc.path }}
            </Button>
            <Button v-else id="close" @click="handleOpen(disc.path)" variant="contained" color="theme" type="submit">
              {{ $t('Eject') }} {{ disc.path }}
            </Button>
          </template>
        ...
        <div v-if="discContents" class="flex justify-between w-inherit h-full overflow-clip gap-4">

          <div class="flex flex-col w-3/5 max-w-3xl h-full overflow-clip">

            <form ref="form" v-if="discContents" @submit="submit"
                  class="flex flex-col h-inherit w-full overflow-clip relative">
              <div class="mb-6 flex w-full">
                <h1 class="text-lg font-bold">
                  {{ discContents.label }}
                </h1>
                <Button id="rescan" variant="contained" color="theme" type="submit"
                        class="ml-auto rounded-md px-1.5 text-sm">
                  {{ $t('Rip') }}
                </Button>
              </div>

              <div class="flex flex-col w-inherit flex-1 h-px overflow-clip relative">
                <ScrollContainer :static="false" autoHide>
                  <div class="flex flex-col w-available h-auto pr-6">
                    <template v-for="(playlist, playlistIndex) in discContents.metadata.bluRay_playlists" :key="playlistIndex">
                      <div class="mb-8 first:mb-0 last:mb-0 to-slate-light-6 dark:to-slate-dark-5 w-full-h-full rounded-xl p-0.5 overflow-clip z-10"
                           style="--tw-gradient-from: hsl(from rgb(var(--color-focus, var(--color-theme-9))) h s calc(l / 2)) var(--tw-gradient-from-position)"
                            :class="{
                                'bg-slate-light-6 dark:bg-slate-dark-5': playlistIndex > 0,
                                'bg-gradient-to-t from-focus/50': playlistIndex === 0
                            }"
                      >
                        <div class="rounded-[10px] w-full bg-slate-light-4 dark:bg-slate-dark-2">
                          <div class="mb-1 w-full flex items-center overflow-clip p-4">
                            <h2 class="font-semibold whitespace-nowrap">
                              {{ $t('Playlist') }}: {{ playlist.playlist_id }}
                            </h2>
                            <Select v-model="episodes[playlistIndex]" :options="discContents.metadata.data.Episodes"
                                    :optionLabel="(episode: Episode) =>
                                    ...
                                    variant="filled" size="small" class="w-px flex-1 mx-4"/>
                            <Button v-if="ripperPlaylist && ripperPlaylist?.playlist_id == playlist.playlist_id"
                                    type="button" id="kill" variant="contained" color="theme" startIcon="stop"
                                    @click="handleStop" :disabled="playButtonLocked">
                              {{ $t('Playing') }}
                            </Button>
                            <Button v-else type="button" id="play" variant="contained" color="theme" startIcon="play"
                                    @click="handlePlay(playlist)" :disabled="playButtonLocked">
                              {{ $t('Play') }}
                            </Button>
                          </div>
                          ...
                          <div class="flex gap-2 w-full p-4 pt-0">
                            <div class="flex flex-col gap-2 w-full">
                              <div class="">
                                <Button type="button" id="rescan" variant="contained" color="theme"
                                        class="rounded-md px-1.5 text-sm" @click="handleSelect(playlist)">
                                  {{ $t(playlist.video_tracks?.some(p => p.checked) ||
                                    playlist.audio_tracks?.some(p => p.checked) ||
                                    playlist.subtitle_tracks?.some(p => p.checked) ? 'Deselect all' : 'Select all') }}
                                </Button>
                              </div>
                              <div class="">
                                ...
                                <ul class="flex flex-col gap-1">
                                  <li v-for="(video, videoIndex) in playlist.video_tracks ?? []" :key="videoIndex"
                                    ...
                                    <Checkbox
                                        ...
                                        :inputId="`playlists[${playlist.playlist_id}].video_tracks[${videoIndex}]`"
                                        class="mr-2 accent-focus size-4"
                                        v-model="video.checked"
                                        @update:modelValue="() => handleChecked(playlist)"
                                    />
                                    <label :for="`playlists[${playlist.playlist_id}].video_tracks[${videoIndex}]`">
                                      {{ video.format }} - {{ video.width }}x{{ video.height }} -
                                      {{ video.display_aspect_ratio }} - {{ video.frame_rate }} FPS
                                    </label>
                                  </li>
                                </ul>
                              </div>
                              <div class="">
                                ...
                                <ul class="flex flex-col gap-1">
                                  <li v-for="(audio, audioIndex) in playlist.audio_tracks ?? []" :key="audioIndex"
                                      class="text-2xs flex items-center">
                                    <Checkbox
                                        binary size="small"
                                        :id="`playlists[${playlist.playlist_id}].audio_tracks[${audioIndex}]`"
                                        :name="`playlists[${playlist.playlist_id}].audio_tracks[${audioIndex}]`"
                                        :inputId="`playlists[${playlist.playlist_id}].audio_tracks[${audioIndex}]`"
                                        class="mr-2 accent-focus size-4"
                                        v-model="audio.checked"
                                        @update:modelValue="() => handleChecked(playlist)"
                                    />
                                    <label :for="`playlists[${playlist.playlist_id}].audio_tracks[${audioIndex}]`">
                                      {{ audio.format }} - {{ audio.language }} -
                                      {{ audio.channels ? audio.channels : 'Unknown' }} Channels
                                    </label>
                                  </li>
                                </ul>
                              </div>
                              <div class="">
                                ...
                                <ul class="flex flex-col gap-1">
                                  <li v-for="(subtitle, subtitleIndex) in playlist.subtitle_tracks ?? []"
                                      :key="subtitleIndex"
                                      class="text-2xs flex items-center">
                                    <Checkbox
                                        binary size="small"
                                        ...
                                        :inputId="`playlists[${playlist.playlist_id}].subtitle_tracks[${subtitleIndex}]`"
                                        class="mr-2 accent-focus size-4"
                                        v-model="subtitle.checked"
                                        @update:modelValue="() => handleChecked(playlist)"
                                    />
                                    <label :for="`playlists[${playlist.playlist_id}].subtitle_tracks[${subtitleIndex}]`">
                                      {{ subtitle.format }} - {{ subtitle.language }}
                                    </label>
                                  </li>
                                </ul>
                              </div>
                            ...
                            <div class="flex flex-col gap-2 w-full mt-10">
                              ...
                              <div class="">
                                ...
                                <ul class="flex flex-col gap-1">
                                  <li v-for="(chapter, chapterIndex) in playlist.chapters" :key="chapterIndex"
                                      class="text-2xs">
                      ...
                      <div class="flex flex-nowrap mb-8 items-center w-available relative gap-1 px-4 pt-5 pb-1 rounded-b-xl -mt-4 z-0"
                           style="background-color: hsl(from rgb(var(--color-focus, var(--color-theme-9))) h s calc(l / 2));"
                           :class="{
            ...
            <div class="h-available w-full overflow-auto" v-else>
