...
import {DesktopUIPlugin, KeyHandlerPlugin, nmplayer, type NMPlayer, type PlayerConfig} from "@/lib/VideoPlayer";
import {discContents, ripperPlaylist, ripperPlaylistLoading} from "@/store/ripper";
      ...
      const config: PlayerConfig = {
        ...
        playlist: [{
            ...
            image: discContents.value?.metadata.data.TvShow?.backdrop_path
                ? 'https://app.nomercy.tv/tmdb-images' + discContents.value?.metadata.data.TvShow?.backdrop_path
                : '',
            description: discContents.value?.metadata.data.TvShow?.overview
                ? 'https://app.nomercy.tv/tmdb-images' + discContents.value?.metadata.data.TvShow?.overview
                : '',
...
<template>
  ...
  >
    ...
    <div class="p-4 h-available w-full overflow-auto absolute inset-0 z-10" v-if="ripperPlaylistLoading">
      <div class="flex flex-col gap-4 items-center justify-center h-full pt-8">
        <svg aria-hidden="true" class="inline w-10 h-10 text-gray-200 animate-spin dark:text-gray-600 fill-focus"
