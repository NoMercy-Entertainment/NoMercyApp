...
const handleType = (value: LogType) => {
  if (selectedTypes.value.some(selectedType => selectedType.name == value.name)) {
    selectedTypes.value = selectedTypes.value.filter(selectedType => selectedType.name != value.name);
  } else {
    selectedTypes.value = [
      ...selectedTypes.value,
      value,
...
const toggleGroup = (group: LogType) => {
  ...
  if (groupLogTypes.every(logType => selectedTypes.value.some(selectedType => selectedType.name === logType.name))) {
    // All log types of the group are already selected, so remove them
    selectedTypes.value = selectedTypes.value.filter(selectedType => !groupLogTypes.some(logType => logType.name === selectedType.name));
  } else {
