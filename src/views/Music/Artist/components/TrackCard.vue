...
<template>
	...
	>
		...
		<div class="flex flex-col items-start justify-start gap-1 self-stretch">
			...
			>
				<div class="w-10 text-center text-sm leading-tight text-auto-10">
					#
				</div>
				...
				<div
					class="shrink grow basis-0 text-right text-sm leading-tight text-auto-10"
				>
					{{ $t("Duration") }}
				</div>
			</div>
		...
		<div class="flex flex-col items-start justify-start gap-1 self-stretch">
			<template v-for="(item, index) in data" :key="item.id + item?.favorite">
				<TrackRow :index="index" :data="item" :display-list="data" />
			</template>
		</div>
	</div>
</template>
