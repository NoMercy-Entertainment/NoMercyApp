...
watch(data, (value) => {
	...
	if (!carousels)
		return;
	selected.value = carousels.find(
		item => item.props.items.length > 0,
	)?.props?.items?.[0]?.props?.data;
});

onIonViewDidEnter(() => {
	const carousels = data.value?.filter(
...
function handleFocus(event: FocusEvent, item?: HomeDataItem) {
