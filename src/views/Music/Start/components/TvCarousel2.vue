  ...
  <div data-row v-if="data.length"
    ...
    <template v-for="(item, index) in data?.slice(0, 7)" :key="item.id">
      <RouterLink :to="`${item.props?.data.link}${suffix}`" :data-card="item.props?.data.id" :data-index="index"
        ...
        @focus="handleFocus($event, item.props.data)" @keydown="handleDown($event)" @keyup="handleUp($event)"
        :style="`background-image: url('${currentServer?.serverBaseUrl}${item.props.data.cover}')`"></RouterLink>
    </template>
  </div>

  <Teleport to=".image-preload-container">
    <template v-for="item in data?.slice(0, 7)" :key="item.id">
      <img :src="`${currentServer?.serverBaseUrl}${item.props.data.cover}`" width="0" height="0" loading="eager" alt=""
        class="absolute">
    </template>
  </Teleport>
</template>
