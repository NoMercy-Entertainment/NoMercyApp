...
export function usePerformanceMonitor() {
  ...
  const measureLoadTime = () => {
    ...
    if (navigation) {
      metrics.value.loadTime = navigation.loadEventEnd - navigation.navigationStart;
  ...
  const logMetrics = () => {
    if (process.env.NODE_ENV === 'development') {
      ...
      console.log('DOM Content Loaded:', metrics.value.domContentLoaded?.toFixed(2), 'ms');
      ...
      console.log('First Input Delay:', metrics.value.firstInputDelay?.toFixed(2), 'ms');
  ...
  const getScores = () => {
    ...
    if (metrics.value.largestContentfulPaint) {
      if (metrics.value.largestContentfulPaint > 4000) {
        ...
        scores.recommendations.push('Optimize Largest Contentful Paint (2.5-4s)');
      }
    }
    ...
    if (metrics.value.cumulativeLayoutShift) {
      if (metrics.value.cumulativeLayoutShift > 0.25) {
        ...
        scores.performance -= 10;
        scores.recommendations.push('Improve Cumulative Layout Shift (0.1-0.25)');
      }
    }
  ...
  const getLoadingMetrics = async () => {
    return {
      ttfb: performance.timing ? performance.timing.responseStart - performance.timing.navigationStart : null,
  ...
  const getResourceMetrics = async () => {
    ...
    const byType: Record<string, { count: number; size: number; loadTime: number }> = {};
    ...
    return {
      totalResources,
      totalSize,
      cacheHitRate: totalResources > 0 ? Math.round((cacheHits / totalResources) * 100) : 0,
      byType
    };
  };
