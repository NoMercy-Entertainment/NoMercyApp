...
class IconOptimizationService {
	...
	private async fetchSVGIcon(
		...
		try {
			...
			const optimizedUrl = `${__dirname}/resources/icons/mooooom/${
				category || 'default'
			}/${iconName}.svg?import`;
			...
			if (!response.ok && category) {
				...
				if (response.ok) {
					...
					if (contentType.includes('text/html')) {
						throw new Error(
							'SVG fallback endpoint returned HTML (SPA fallback detected)',
						);
					}
				}
			}
	...
	private createFontIcon(
		...
		return {
			type: 'font',
			html: `<i class="mooooom-${name} ${className || ''}" style="font-size: ${
	...
	private createSVGIcon(
		...
		const styledSvg = svgContent
			.replace(
				...
				`<svg class="${
					className || ''
				}" width="${sizeStyle}" height="${sizeStyle}" style="${colorStyle}"`,
			)
	...
	private calculateMemorySavings() {
		...
		const svgCacheSize = Array.from(svgIconCache.values()).reduce(
			(total, svg) => total + svg.length,
			0,
		);
		...
		return {
			...
			savedPercentage: (
				((totalPossibleSize - actualUsedSize - svgCacheSize)
					/ totalPossibleSize)
				* 100
			).toFixed(1),
		};
	...
	async generateOptimizedIconFont() {
        ...
        `;
				...
				console.log(
					`Generated optimized icon font with ${usedIconsList.length} icons`,
				);
				return fontUrl;
			}
		}
