...
const props = defineProps({
  ...
  variant: {
    type: String as PropType<'primary' | 'contained' | 'text' | 'outlined' | 'default' | 'hover' | 'active' | 'focussed' | 'disabled' | 'auto'>,
    default: 'default',
    required: false,
  },
  onClick: {
...
<template>
  ...
  <label v-if="component == 'label'"
    ...
    <MoooomIcon v-if="endIcon"
                :icon="endIcon"
                className="-mr-1 w-5"/>
  </label>
  <a v-else-if="component == 'link'"
     :href="href"
     :id="id"
     :data-variant="variant"
     @click="onClick"
