...
function calculateRotation(index: number, total: number) {
	const centerIndex = Math.floor(total / 2);
	return -(index - centerIndex) * (isPlatform('mobile') ? -7 : 7);
}

function calculateTranslation(index: number, total: number) {
	const centerIndex = Math.floor(total / 2);
	const distance = Math.abs(index - centerIndex);
	const maxDistance = centerIndex;
	const factor = isPlatform('mobile') ? -68 : 68;
	return -((distance / maxDistance) ** 2) * factor * maxDistance;
}
</script>
...
<template>
	...
	>
		...
		>
			<div
				class="flex h-full w-auto -translate-x-1/3 flex-nowrap gap-2 sm:gap-16 sm:-translate-x-0"
			>
