...
<template>
  <button v-if="data" :onfocus="scrollIntoView" @click="setData(data)" :style="`--color-focus: ${ringColor};`"
    data-card="true"
    ...
    <span class="relative h-auto w-full cursor-pointer overflow-clip rounded-lg"
      ...
      <TMDBImage className="inset-0 h-full w-full overflow-clip rounded-lg" class="!relative" :aspect="aspect"
        :path="data?.src" type="image" :title="data?.name" :colorPalette="data?.color_palette?.image"
