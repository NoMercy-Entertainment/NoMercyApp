...
<template>
	...
	<div class="flex w-full flex-col gap-4 p-1 h-available sm:p-4" :class="results.length > 0 ? 'mt-4' : ''">
		<div class="flex flex-wrap gap-2 pb-3 sm:gap-4">
			<template v-for="item in results ?? []" :key="item.id">

				<RouterLink :to="`/search/${item?.media_type ?? 'unknown'}/${item?.id ?? 'unknown'}`"
