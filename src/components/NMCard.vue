...
<template>
  <RouterLink v-if="data?.link" :data-scroll="scrollLetter" :data-card="data?.link" @contextmenu="onRightClick($event)"
    ...
    <div class="w-full h-full overflow-clip rounded-lg inset-0 absolute">
      ...
      <TMDBImage :path="image" :title="data.title" loading="lazy" :size="showBackdrops ? 330 : 180"
      ...
      <template v-if="showBackdrops">
        ...
        <div v-if="!!data.logo" class="absolute inset-0 h-full w-full">
          ...
          <div class="absolute bottom-0 left-0 h-full max-h-24 w-full max-w-[66%]">
            <TMDBImage :path="data.logo" :title="data.title" :colorPalette="data.color_palette?.logo" :size="500"
              loading="lazy"
      ...
      <div v-else
        :class="`flex flex-col justify-start items-start w-full h-12 z-0 absolute left-0 transition-all duration-200 px-2 py-1 group-hover/card:-bottom-0 text-left ${image ? '-bottom-20' : 'bottom-0'}`">
        <div
