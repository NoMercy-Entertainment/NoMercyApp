  ...
  <div class="performance-dashboard">
    <div class="dashboard-header">
      ...
      <div class="header-controls">
        <button @click="refreshMetrics" :disabled="isRefreshing" class="refresh-btn">
          {{ isRefreshing ? 'Refreshing...' : 'Refresh' }}
        </button>
        <button @click="clearMetrics" class="clear-btn">Clear Data</button>
      </div>
    </div>
    ...
    <div class="metric-section">
      ...
      <div class="metric-grid">
        <div class="metric-card" :class="getMetricStatus(webVitals.lcp, 2500, 4000)">
          <div class="metric-title">Largest Contentful Paint</div>
          <div class="metric-value">{{ formatTime(webVitals.lcp) }}</div>
          <div class="metric-description">Good: &lt;2.5s, Needs Improvement: 2.5s-4s</div>
        </div>
        
        <div class="metric-card" :class="getMetricStatus(webVitals.fid, 100, 300)">
          <div class="metric-title">First Input Delay</div>
          <div class="metric-value">{{ formatTime(webVitals.fid) }}</div>
          <div class="metric-description">Good: &lt;100ms, Needs Improvement: 100ms-300ms</div>
        </div>
        
        <div class="metric-card" :class="getMetricStatus(webVitals.cls, 0.1, 0.25)">
          <div class="metric-title">Cumulative Layout Shift</div>
          <div class="metric-value">{{ webVitals.cls?.toFixed(3) || 'N/A' }}</div>
          <div class="metric-description">Good: &lt;0.1, Needs Improvement: 0.1-0.25</div>
        </div>
      </div>
    </div>
    ...
    <div class="metric-section">
      ...
      <div class="metric-grid">
        <div class="metric-card">
          ...
          <div class="metric-value">{{ formatTime(loadingMetrics.ttfb) }}</div>
          <div class="metric-description">Server response time</div>
        </div>
        
        <div class="metric-card">
          <div class="metric-title">First Contentful Paint</div>
          <div class="metric-value">{{ formatTime(loadingMetrics.fcp) }}</div>
          <div class="metric-description">First content rendered</div>
        </div>
        
        <div class="metric-card">
          <div class="metric-title">Time to Interactive</div>
          <div class="metric-value">{{ formatTime(loadingMetrics.tti) }}</div>
          <div class="metric-description">Page becomes interactive</div>
        </div>
      </div>
    </div>
      ...
      <div class="resource-metrics">
        <div class="resource-summary">
          ...
          <div class="summary-item">
            ...
            <span class="value">{{ formatBytes(resourceMetrics.totalSize) }}</span>
          </div>
          <div class="summary-item">
        ...
        <div class="resource-breakdown">
          ...
          <div class="breakdown-grid">
            <div v-for="(resource, type) in resourceMetrics.byType" :key="type" class="breakdown-item">
              <div class="resource-type">{{ type }}</div>
              <div class="resource-count">{{ resource.count }} files</div>
              <div class="resource-size">{{ formatBytes(resource.size) }}</div>
              <div class="resource-time">{{ formatTime(resource.loadTime) }}</div>
            </div>
          </div>
        </div>
      </div>
    ...
    <!-- Recommendations -->
    <div class="metric-section" v-if="recommendations.length > 0">
    ...
    <div class="metric-section">
      ...
      <div class="realtime-metrics">
        <div class="realtime-item">
          ...
          <span class="value" :class="getFPSStatus(currentFPS)">{{ currentFPS }}</span>
        </div>
        <div class="realtime-item">
...
const memoryUsagePercentage = computed(() => {
  if (memoryMetrics.value.jsHeapSizeLimit === 0) return 0;
  return Math.round((memoryMetrics.value.usedJSHeapSize / memoryMetrics.value.jsHeapSizeLimit) * 100);
});
...
const getMetricStatus = (value: number | null, goodThreshold: number, needsImprovementThreshold: number) => {
...
const generateRecommendations = () => {
  ...
  if (memoryUsagePercentage.value > 80) {
    newRecommendations.push({
      ...
      description: 'Consider implementing virtual scrolling and cleaning up unused resources',
      impact: 'Medium',
  ...
  if (resourceMetrics.value.totalSize > 5 * 1024 * 1024) { // 5MB
    newRecommendations.push({
  ...
  if (resourceMetrics.value.cacheHitRate < 70) {
    newRecommendations.push({
      ...
      description: 'Optimize service worker caching and implement better cache headers',
      impact: 'Medium',
      priority: 'medium' as const
    });
  }
...
const updateMemoryChart = () => {
  ...
  // Used memory
  ctx.fillStyle = usedPercentage > 0.8 ? '#ef4444' : usedPercentage > 0.6 ? '#f59e0b' : '#10b981';
...
const updateTimelineChart = () => {
  ...
  metrics.forEach((metric, index) => {
    if (metric.value) {
      ...
      const isDarkMode = document.documentElement.classList.contains('scheme-dark');
...
.resource-type, .resource-count, .resource-size, .resource-time {
  @apply text-slate-lightA-11 dark:text-slate-darkA-11;
}
