...
<template>
  <RouterLink v-if="data && data?.type !== 'playlists'" :to="data?.link" :style="`
  ...
  <RouterLink v-else-if="data?.type == 'playlists'" :to="data?.link" :style="`--color-focus: ${ringColor}`"
    ...
    <div class="relative flex w-28 min-w-28 flex-grow-0 items-center justify-start gap-2 rounded-xl flex-shrink-0-0">
      <MusicCardImage :data="data" />
    </div>
