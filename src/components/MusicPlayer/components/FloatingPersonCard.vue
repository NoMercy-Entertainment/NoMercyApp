    ...
    <div class="sticky top-14 flex h-auto w-full flex-col gap-4">
      <div :id="data?.id"
        ...
        <div class="relative flex h-full w-full flex-col">
          <TMDBImage :path="currentImage?.src ?? data?.poster"
                     :size="180"
      ...
      <div class="relative grid grid-cols-4 gap-2 group/personCard">
        <div v-for="profile in data?.images?.profiles?.slice(0, 4)"
             :key="profile.src"
          ...
          <div
              :class="profile.src == currentImage?.src ? 'opacity-0' : 'opacity-100'"
