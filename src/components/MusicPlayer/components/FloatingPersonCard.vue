...
<template>
	...
	>
		<div class="sticky top-28 flex h-auto w-full flex-col gap-4">
			<div
				:id="data?.id ?? 'x'"
				:class="data?.deathday ? 'outline outline-white' : ''"
				class="relative z-0 block w-auto select-none overflow-clip rounded-2xl transitioning aspect-poster"
			>
				<div class="relative flex h-full w-full flex-col">
					<TMDBImage
						:path="currentImage?.src ?? data?.poster"
						:size="180"
						:color-palette="data?.color_palette?.profile"
						class-name="pointer-events-none absolute inset-0 z-20 flex h-full w-full object-cover transition-transform duration-500 trickle-div"
					/>
				</div>
			</div>
			<div class="relative grid grid-cols-4 gap-2 group/personCard">
				<div
					v-for="profile in data?.images?.profiles?.slice(0, 4)"
					:key="profile.src"
				...
				>
					<div
						:class="
							profile.src === currentImage?.src ? 'opacity-0' : 'opacity-100'
						"
					...
					<TMDBImage
						...
						:color-palette="data?.color_palette?.profile"
						:size="182"
						class-name="relative z-0 block h-auto w-full select-none transitioning aspect-poster"
					/>
				</div>
			</div>
		</div>
