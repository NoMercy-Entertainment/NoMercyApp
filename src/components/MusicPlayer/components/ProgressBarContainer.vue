...
<template>
  <div class="flex items-center justify-center !w-full flex-row select-none monospace" :data-size="musicSize"
    ...
    <div v-if="variant == 'stacked'" class="flex w-full flex-col">
      ...
      <div class="flex w-full items-center justify-between">
        <div class="music-compact:pointer-events-none pointer-events-auto flex h-full items-center w-available"
          :data-size="musicSize">
          <ProgressBar :color="color" :onKeyDown="onKeyDown" :onKeyUp="onKeyUp" />
        </div>
      </div>
      ...
      <div class="flex w-full items-center justify-between">
        ...
        <span class="mx-2 !text-contrast whitespace-nowrap text-xs relative flex justify-between monospace"
          :data-size="musicSize" @click="toggleTimeChoice">
          <span aria-label="duration" class="" v-if="timeChoice === 'duration'" :data-size="musicSize">
            {{ humanTime(duration) }}
          </span>
    ...
    <div v-else class="flex w-full items-center">
      ...
      <span class="mx-2 !text-contrast whitespace-nowrap text-xs relative flex justify-between" :data-size="musicSize"
