...
import { GenreItem } from '@/types/api/music/genres';
...
const ringColor = ref(pickPaletteColor(props.data?.color_palette?.poster)
  ?.replace('rgb(', '')
  .replace(')', '')
...
<template>
  <RouterLink v-if="data"
    ...
    :to="data?.link" :id="data?.id" data-card="true" :style="`--color-focus: ${ringColor};`"
    :data-scroll="`scroll_${data?.name?.[0]?.toUpperCase()}`" data-target="navigate">
    <div
      class="relative flex h-full flex-col overflow-clip rounded-lg bg-frame shadow-card transitioning group-hover:bg-auto-200">
      <div class="pointer-events-none absolute inset-0 z-10 aspect-square h-auto w-full rounded-lg shadow-top"></div>
