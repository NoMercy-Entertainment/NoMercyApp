  ...
  <aside :class="{
    ...
    <div ref="container"
      ...
      <div v-if="currentServer"
        ...
        <SideFlyoutButton icon="serverSwitch" title="Switch server" href="/setup/select-servers"
          @click="closeSidebar" />
      </div>
      ...
      <div v-if="currentServer"
        ...
        <SideFlyoutButton icon="home1" title="Home" href="/home" @click="closeSidebar" v-if="shouldShowLibraryLinks"
          :active="isHomeRoute" />
        <SideFlyoutButton icon="folder" title="Libraries" href="/libraries" @click="closeSidebar"
          v-if="shouldShowLibraryLinks && hasMusicLibrary" :active="isLibraryRoute" />
        <SideFlyoutButton icon="noteDouble" title="Music" href="/music/start" @click="closeSidebar"
          v-if="shouldShowLibraryLinks && hasMusicLibrary" :active="isMusicRoute" />
      </div>
      <div class="flex flex-shrink-0 flex-grow-0 flex-col items-start justify-start gap-1 self-stretch  px-3 py-2">
        <SideFlyoutButton v-if="currentServer" icon="gridMasonry" title="System" href="/dashboard/system"
          :active="isDashboardRoute" @click="closeSidebar" />
        <SideFlyoutButton v-if="currentServer" icon="paintSwatches" title="Preferences" :active="isPreferencesRoute"
          @click="closeSidebar" href="/preferences/display" />
        <SideFlyoutButton icon="arrowRefreshHorizontal" title="Refresh" :onclick="reload" @click="closeSidebar" />

        <SideFlyoutButton icon="moonDiagonal" title="Dark mode" @click="darkMode = !darkMode">
          <label class="inline-flex w-min cursor-pointer items-center gap-3" for="ion-tg-0">
