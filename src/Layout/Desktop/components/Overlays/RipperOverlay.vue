...
import {ripperSocketIsConnected, startRipperSocket, stopRipperSocket, connection} from "@/lib/clients/ripperSocket";
...
const handleRip = (disc: DriveState) => {
...
watch(ripperSocketIsConnected, async (value) => {
  if (value) {
    ...
    for (const drive of discs.value ?? []) {
      ...
      if (!drive.label) continue;
      connection.value?.invoke('GetDriveState', drive.path)
...
<template>
  <div :data-open="ripperMenuOpen" id="ripperContainer" :inert="!ripperMenuOpen"
    ...
    <div class="flex items-center justify-center h-full" v-if="discs.some(disc => !disc.open && !disc.metadata)">
