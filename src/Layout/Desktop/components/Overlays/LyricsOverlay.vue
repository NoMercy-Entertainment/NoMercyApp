...
import audioPlayer, {currentSong, currentTime, setHasLyrics, lyricsMenuOpen} from '@/store/audioPlayer';
...
<template>
    <div :data-open="lyricsMenuOpen"
        class="absolute inset-0 h-inherit w-inherit sm:left-auto sm:right-4 sm:top-4 sm:bottom-4 sm:w-2/3 sm:max-w-3xl flex items-center justify-center rounded-xl transition-all duration-300 sm:data-[open='false']:translate-x-[150%] sm:overflow-clip bg-focus z-[9999]">
        <div
            class="pointer-events-none absolute inset-0 z-0 flex items-center justify-center rounded-xl bg-black opacity-40">
        </div>

        <div ref="lyrics_container" :id="`lyrics_container_${id}`"
             class="relative z-10 flex w-full flex-col gap-2 overflow-auto tv:overflow-clip p-4 children:transition-all duration-300 h-available font-bbc sm:gap-4 sm:p-16 sm:text-2xl">
            ...
            <template v-if="!!lyrics">
                <div class="contents" v-if="Array.isArray(lyrics)">
                    ...
                    <p class="mt-16 font-semibold text-2xs">
