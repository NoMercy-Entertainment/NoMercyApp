...
<template>
  <aside id="sidebar" :data-sidebar="sidebar"
    ...
    <div
      ...
      >
        <div class="flex flex-col items-start justify-start gap-1 self-stretch px-4 py-1">
          ...
          <!--                    Music pages-->
          <SidebarButton href="/music/start" :show="isMusicRoute" icon="speaker" name="Start" />
          <SidebarButton href="/music/artists" :show="isMusicRoute" icon="user" name="Artists" />
          <SidebarButton href="/music/albums" :show="isMusicRoute" icon="disk" name="Albums" />
          <SidebarButton href="/music/genres" :show="isMusicRoute" icon="noteClefTreble" name="Genres" />
          <SidebarButton href="/music/tracks" :show="isMusicRoute" icon="heart" name="Songs you like" />

          <SidebarButtonGroup :show="isMusicRoute" icon="playlist1Add" v-if="playlists?.length ?? 0 > 0"
            title="Playlists">

            <SidebarButton href="/music/playlists" v-if="playlists?.length ?? 0 > 0" name="Show all" />

            <template v-for="playlist in playlists ?? []" :key="playlist.id">
              <SidebarButton :href="`/music/playlists/${playlist.id}`" :cover="playlist.cover" :name="playlist.name" />
            </template>
          ...
          <SidebarButtonGroup :show="isDashboardRoute" title="System">
            ...
            <SidebarButton href="/dashboard/system" icon="gridMasonry" name="System info" />
            <SidebarButton href="/dashboard/general" icon="settings" name="General" />
            <SidebarButton href="/dashboard/users" icon="users" name="Users" />
            <SidebarButton href="/dashboard/libraries" icon="folder" name="Libraries" />
            <SidebarButton href="/dashboard/encoderprofiles" icon="playCircle" name="Encoder profiles" />
            <SidebarButton href="/dashboard/notifications" icon="bell" name="Notifications" />
          ...
          <SidebarButtonGroup :show="isDashboardRoute" title="Content">
            ...
            <SidebarButton href="/dashboard/specials" icon="sparkles" name="Specials" />
            <SidebarButton href="/dashboard/ripper" icon="disk" name="Ripper" />

          </SidebarButtonGroup>
          <SidebarButtonGroup :show="isDashboardRoute" title="Devices">

            <SidebarButton href="/dashboard/devices" icon="monitor" name="Devices" />
            <SidebarButton href="/dashboard/activity" icon="tag" name="Activity" />
          ...
          <SidebarButtonGroup :show="isDashboardRoute" title="Advanced">
            ...
            <SidebarButton href="/dashboard/logs" icon="terminalBox" name="Logs" />
