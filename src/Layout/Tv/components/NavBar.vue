  ...
  <ion-header v-if="navBarVisible" mode="md" class="pt-safe transition-all duration-200 !h-[104px]">
    <ion-toolbar mode="md">
      <div class="flex justify-between items-center w-full relative px-14 h-14">
        ...
        <button v-if="currentSong" @click="openFullPlayer()"
          ...
          <div class="relative w-10 h-10 aspect-square rounded-full overflow-clip">
            <CoverImage :data="currentSong" :alt="currentSong.name"
              class="w-auto h-full aspect-square absolute top-0 left-0 brightness-50" :class="{
                '': isPlaying,
              }" />
          ...
          <div
            class="w-[calc(100%-30px)] flex flex-col justify-center items-start gap-1.5 mr-2 overflow-clip py-0.5 border-[4px] border-l-0 rounded-r-full border-slate-dark-6">
